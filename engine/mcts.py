...
class SimpleNode:
    ...
    def best_action(self):
        """Choose the action with highest value among safe actions that don't lead to being trapped."""
        ...
        # If we have trap-free actions, use those
...
class PureMCTS:
    ...
    # NEW: Add parallelized version of select_action
    def select_action_parallel(self, game, num_workers=4):
        ...
        Returns:
            The action with the most votes across all parallel searches
        """
        legal_actions = game.get_legal_actions(self.player_id)
        ...
        # Run parallel searches using ProcessPoolExecutor
    ...
    def simulate(self, state):
        ...
        while not sim_state.is_terminal() and depth < self.max_depth:
            ...
            # During simulation, no safety checks - just random actions
    ...
    def evaluate(self, state):
        ...
        # Calculate estimated future box reward
    ...
    def estimate_future_boxes_destroyed(self, state):
        ...
        Simulates 8 turns where players "vanish" (no new bombs placed).
        Adds gamma^d for each box destroyed, where d is the turn when destroyed.
        """
        future_state = state.copy()
        
        # Make all players "vanish" - they can't place new bombs
        ...
        for d in range(8):
            ...
            # Update bombs only (players don't act)
            ...
            # Add reward: gamma^d for each box destroyed in turn d
            total_future_reward += boxes_destroyed_this_turn * (gamma ** (d + 1))
        
        # Scale the future reward to be similar to other reward components
    ...
    def is_safe_action(state, player_id, action, max_depth=8):
        """
        Check if taking an action leads to a safe state where the player can survive
        for the next max_depth turns, assuming opponents don't place new bombs.
        Uses 8-turns deep BFS to find a safe sequence of actions.
        Returns True if a safe path exists, False otherwise.
        """
        ...
        # Start with depth 1 since we've already completed one turn
        queue = deque([(test_state, 1)])
        
        # Track visited states to avoid cycles
        # State is identified by: (player_pos, depth, bomb_state)
        visited = set()
        ...
        while queue:
            ...
            for next_action in legal_actions:
                # Skip BOMB action - we're checking for safety, not placing bombs
                ...
                # Complete the round (other players' turns)
    ...
    def can_be_trapped_by_enemy(state, player_id, max_trap_turns=2, max_kill_turns=10):
        """
        Check if any enemy player can trap and eventually kill our agent within the specified turns.
        
        Args:
            state: The current game state
            player_id: ID of our agent
            max_trap_turns: Maximum turns to check for trapping (default 2)
            max_kill_turns: Maximum turns to check for killing after trapping (default 10)
            
        Returns:
            True if agent can be trapped and killed, False otherwise
        """
        # If player is already dead, they're not trappable
        ...
        for enemy_id in range(state.num_players):
            if enemy_id != player_id and state.player_alive[enemy_id]:
                ...
                queue = deque([(enemy_pos, 0)])  # (pos, steps)
                
                while queue:
                    (ex, ey), steps = queue.popleft()
                    
                    if steps <= max_trap_turns:
                        # This is a position where the enemy could place a bomb
                ...
                # For each potential bomb position, check if placing a bomb there would trap our agent
                ...
                for bx, by in potential_bomb_positions:
                    ...
                    # Check if our agent can find safety
